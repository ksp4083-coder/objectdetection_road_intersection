...
- YOLO의 경우 YOLO 형식 객체 위치 정보가 담긴 폴더를 학습 데이터셋 폴더 하위에 위치시키고 학습 이미지 경로를 yaml 파일에 지정해서 모델이 객체의 특성을 학습할 수 있도록 함
  - YOLO는 딥러닝 기반 모델로 모델을 학습하는 동시에 검증을 수행함
  - 검증 주기는 전체 데이터셋을 한 번 학습할 때 마다 검증을 수행 (1 epoch 주기)
  - 이러한 이유로 모델 검증에 사용할 이미지에 대한 YOLO 형식 객체 위치 정보 파일도 생성해 주어야 하며
  - 학습 데이터셋과 동일하게 검증 데이터셋 폴더 하위에 위치시키고 검증 이미지 경로를 yaml 파일에 지정해 주어야 모델 학습을 시작할 수 있음 
- yaml 파일 예시 <br>
<p align="center">
...
## 데이터 전처리
### 1. 7,000개의 객체 위치 정보 파일 생성
- YOLO 모델을 학습하기 위해 train.json 파일에서 이미지 7,000 장에 대한 이미지 정보와 객체 위치 정보를 담고 있는 json 파일 7,000개를 생성함
- 생성한 json 파일은 <ins>*data_road_intersection/labels_json*</ins> 폴더에 저장함
- 이미지 별 객체 위치 정보 파일(.json) 예시 <br>
  ...
  - 정답지는 객체 위치 정보를 의미하며 모델 학습과 검증에 사용할 수 있는 객체 수를 확인함
...
- YOLO의 경우 YOLO 형식 객체 위치 정보가 포함된 파일의 경로를 yaml 파일에 지정해서 모델이 객체의 특성을 학습할 수 있도록 함 <br><br>
  ...
  #### 2-1. 변수 추출
  - 우리가 사용하는 데이터에서 객체 위치 정보는 Bounding Box의 좌측 상단 좌표(x, y)와 가로길이(w), 세로길이(h)로 제공됨
  - 해당 정보는 json 파일 내부 annotations 변수의 annotation_info 변수에 들어있음
