...
- 모델 평가는 상대 평가로 [정확도 만점 기준 30점, 추론시간 만점 30점, 그리고 모델 개발 창의성 부문 40]점 이었음
- **이번 대회에서 20개의 YOLO 모델을 개발**했고, 최종적으로 모델 평가에 활용될 하나의 모델을 선정해서 제출해야 됐기 때문에 **정확도와 추론시간에 기반해 하나의 모델을 선정하는 방식을 개발함**
